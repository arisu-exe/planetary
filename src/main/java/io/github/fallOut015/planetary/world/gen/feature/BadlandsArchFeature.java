...
public class BadlandsArchFeature extends Feature<NoFeatureConfig> {
    public BadlandsArchFeature(Codec<NoFeatureConfig> codec) {
        super(codec);
    }
    ...
    public boolean place(ISeedReader reader, ChunkGenerator generator, Random rand, BlockPos pos, NoFeatureConfig config) {
        ...
        for(double x = 0; x < w + 1; ++ x) {
            ...
            if(x == 0) {
                ...
                while(!reader.getBlockState(position = position.below()).canOcclude()) { }

                BlockPos.betweenClosedStream(position, pos1.below()).forEach(blockPos -> {
                    if(reader.isEmptyBlock(blockPos)) {
                        reader.setBlock(blockPos, Blocks.SMOOTH_RED_SANDSTONE.defaultBlockState(), 0);
                    }
                });
            } else {
