...
public class MultiReplaceBlockConfig implements IFeatureConfig {
    public static final Codec<MultiReplaceBlockConfig> CODEC = RecordCodecBuilder.create((instance) ->
        instance.group(
            BlockState.CODEC.fieldOf("target1").forGetter((config) -> config.target1),
            BlockState.CODEC.fieldOf("state1").forGetter((config) -> config.state1),
            BlockState.CODEC.fieldOf("target2").forGetter((config) -> config.target2),
            BlockState.CODEC.fieldOf("state2").forGetter((config) -> config.state2)
